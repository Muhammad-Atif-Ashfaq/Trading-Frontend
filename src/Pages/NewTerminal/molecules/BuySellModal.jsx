...
import { setOrderRefresh } from '../../../store/terminalSlice';
  ...
  const createOrder = async (typeReceive, skip=false) => {
    ...
    try {
      ...
      const SymbolData = {
        ...
        open_time: moment().format('MM/DD/YYYY hh:mm:ss A'),
        brand_id,
      ...
      if (success) {
        ...
        CustomNotification({ type: "success", title: "Trade Order", description: message, key: 1 })
        ...
        setIsLoading(false)
        CustomNotification({ type: "error", title: "Trade Order", description: message, key: 1 })
        if (trade_type === "group") {
          setIsModalOpen(true)
      ...
      CustomNotification({ type: "error", title: "Trade Order", description: err.message, key: 1 })
      ...
      const res = (parseFloat(parseFloat(volume) * parseFloat(selectedWatchMarket?.lot_size) * open_price).toFixed(2));
        ...
        if(margin > equity) {
          CustomNotification({ 
            ...
            createOrder(typeReceive)
            :
            CustomNotification({ type: "error", title: "Live Order", description: `Insufficient Balance. Your balance should be greater than $${ parseFloat(calculatedMargin.toFixed(2))} but you have $${balance}`, key: 1 })
          }
      }
      else 
      {
